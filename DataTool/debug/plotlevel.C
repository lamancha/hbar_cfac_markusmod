...
void plotlevel(int n = 4, int l = 2, int twoj = 5)
{
  TString LevelFile = "../output/output_point_p.root";
  rfacInterpolation * myInterpolation = new rfacInterpolation(LevelFile);
  vector<TGraph *> gr;
  ...
  for(vector<QuantumNumbers *>::const_iterator it = target.begin(); it!=target.end(); ++it)
      ...
      for(int j = 0; j<N; ++j)
	{
	  ...
	  vx.push_back(Bfield);
	  vy.push_back(EV_TO_HZ/1E9*(myInterpolation->GetEnergy(0, Bfield, 0)-myInterpolation->GetEnergy(0, 0, 0))); ///
	}
